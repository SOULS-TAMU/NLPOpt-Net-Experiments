...
def _model_color(series_label: str) -> str:
    ...
    if normalized == "dc3":
        return "#1F77B4"
    if normalized in {"baseline nn", "baseline", "baseline_nn"}:
        return "#2CA02C"
    if normalized in {"eqnn", "baseline_eq_nn", "baseline eq. nn", "baseline eq nn"}:
        return "#D62728"
    return "#4B5563"
...
def save_comparison_shadow_plot(
    output_path: Path,
    *,
    epochs: Sequence[int],
    ...
    if len(epochs) == 0:
        raise ValueError("Expected at least one history point to plot.")
    if not model_runs:
        raise ValueError("Expected at least one model entry for comparison plotting.")
...
def save_comparison_shadow_objective_value_violation_plot(
    output_path: Path,
    *,
    epochs: Sequence[int],
    model_runs: Mapping[str, Mapping[str, Sequence[Sequence[float]]]],
    ...
    for series_label, payload in model_runs.items():
        ...
        train_obj_arr = _as_epoch_run_matrix(payload["train_objective_runs"], epochs, name=f"{series_label}.train_objective_runs")
        val_obj_arr = _as_epoch_run_matrix(payload["val_objective_runs"], epochs, name=f"{series_label}.val_objective_runs")
        train_violation_arr = _as_epoch_run_matrix(payload["train_violation_runs"], epochs, name=f"{series_label}.train_violation_runs")
        val_violation_arr = _as_epoch_run_matrix(payload["val_violation_runs"], epochs, name=f"{series_label}.val_violation_runs")

        if val_obj_arr.shape != train_obj_arr.shape:
            raise ValueError(f"{series_label} validation objective runs must match training objective shape.")
        if train_violation_arr.shape != train_obj_arr.shape:
            raise ValueError(f"{series_label} training violation runs must match training objective shape.")
        if val_violation_arr.shape != train_obj_arr.shape:
            raise ValueError(f"{series_label} validation violation runs must match training objective shape.")
        ...
        for arr in (train_obj_arr, val_obj_arr):
            finite_values = arr[np.isfinite(arr)]
            if finite_values.size > 0:
                finite_objective_chunks.append(finite_values)
    ...
    ax1.xaxis.set_major_formatter(LogFormatterMathtext(base=10))
    ax1.xaxis.set_minor_formatter(NullFormatter())
    if finite_objective_chunks:
        finite_objective = np.concatenate(finite_objective_chunks)
        obj_min = float(np.min(finite_objective))
        obj_max = float(np.max(finite_objective))
        if obj_min == obj_max:
            pad = max(1.0, abs(obj_min)) * 0.05
