...
from scripts.plot_utils.plotting import save_comparison_shadow_objective_value_violation_plot
from scripts.testcase import general_run_dc3 as dc3_runner
from scripts.testcase import general_run as local_runner
from scripts.testcase import unified_runner as unified

_FRAMEWORKS = ("nlpopt", "dc3")
_TABLE_COLUMNS = (
...
def run_comparison(_case_dir: Path | None = None, _path_arg: str | None = None) -> int:
    case_dir = local_runner._resolve_workspace(str(_case_dir) if _case_dir is not None else None)
    data_cfg, cfg_dict, proj_cfg, model_def_path = local_runner._load_local_configs(case_dir)
    data_cfg = local_runner._normalize_local_data_cfg(data_cfg, model_def_path)
    dataset_dir = local_runner.dataset_dir(case_dir, data_cfg, model_def_path)
    ...
    for framework in _FRAMEWORKS:
        ...
        print("")
        print("=" * 80)
        print(f"Comparison runner | framework={unified._framework_label(framework)}")
        print(f"Dataset target: {dataset_dir}")
        print(f"Seeds: {base_seed}..{base_seed + num_seeds - 1} ({num_seeds} runs)")
        print("=" * 80)

        if framework == "nlpopt":
            local_runner._run_multi_seed(case_dir, data_cfg, run_cfg, proj_cfg, model_def_path)
        elif framework == "dc3":
            dc3_runner._run_multi_seed(case_dir, data_cfg, run_cfg, proj_cfg, model_def_path)
        else:
            raise ValueError(f"Unsupported general comparison framework '{framework}'.")

        multi_dir = unified._framework_multi_dir(dataset_dir, framework)
        ...
        framework_manifests[framework] = {
            "label": unified._framework_label(framework),
    ...
    (comparison_dir / "model_definition.py").write_text(model_def_path.read_text(encoding="utf-8"), encoding="utf-8")
...
def plot_root_comparison(dataset_dir_arg: str | None = None) -> int:
    ...
    save_comparison_shadow_objective_value_violation_plot(
        output_path,
        epochs=epochs,
    ...
    if not cfg_path.exists():
        cfg_path = Path(__file__).resolve().parents[2] / "case" / "general" / "config.json"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate the general-case comparison shadow plot and summary CSVs.")
