...
from scripts.testcase import standard as standard_case
...
def _run_framework_comparison(
    ...
    *,
    ...
    data_cfg, cfg_dict, proj_cfg = standard_case.load_local_configs(case_dir)
    dataset_dir = unified._resolve_dataset_dir(case_dir, data_cfg)
    if comparison_dir_name == "comparison_dc3":
        comparison_dir = dataset_dir / comparison_dir_name / _train_frac_dir_name(cfg_dict)
    else:
        comparison_dir = (dataset_dir / comparison_dir_name) if comparison_dir_name else unified._comparison_dir(dataset_dir)
    ...
    for framework in frameworks:
        ...
        print(unified._problem_shape_text(data_cfg))
        ...
        unified._run_multi_seed(case_dir, data_cfg, run_cfg, proj_cfg, multi_dir_override=framework_multi_dir)

        multi_dir = Path(framework_multi_dir) if framework_multi_dir is not None else unified._framework_multi_dir(dataset_dir, framework)
        summary_path = multi_dir / "multi_seed_summary.json"
        if not summary_path.exists():
            raise FileNotFoundError(f"Expected multi-seed summary at {summary_path}")
        summary_payload = unified._load_json(summary_path)
...
def plot_comparison(case_dir: Path, dataset_dir_arg: str | None = None) -> int:
    target = dataset_dir_arg if dataset_dir_arg is not None else str(standard_case.default_dataset_dir(case_dir))
...
def plot_time_comparison(case_dir: Path, comparison_dir_arg: str | None = None) -> int:
    target = comparison_dir_arg if comparison_dir_arg is not None else str(standard_case.default_comparison_dir(case_dir))
