...
def run_case(
    ...
    *,
    ...
    if float(timing_summary["backbone_total_sec"]) + float(timing_summary["projection_total_sec"]) + float(timing_summary["backward_total_sec"]) + float(timing_summary["optimizer_total_sec"]) > 0.0:
        print(
            f"Backbone forward : {float(timing_summary['time_backbone_percent']):6.2f}%"
            " (train + val)"
        )
        print(
            f"Projection       : {float(timing_summary['time_projection_percent']):6.2f}%"
            " (train + val)"
        )
        print(
            f"Backward         : {float(timing_summary['time_backward_percent']):6.2f}%"
            " (train only)"
        )
        print(
            f"Optimizer update : {float(timing_summary['time_optimizer_percent']):6.2f}%"
            " (train only)"
        )
    print("")
