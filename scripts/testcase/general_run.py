...
from case.general.factory import expand_parameter_bounds, model_definition_path, parameter_size  # noqa: E402
...
def _normalize_local_data_cfg(data_cfg: Mapping[str, Any], model_def_path: Path) -> dict[str, Any]:
    if "num_samples" in data_cfg:
        ...
        x_l_raw = data_cfg["x_L"]
        x_u_raw = data_cfg["x_U"]
    else:
    ...
    n_x = parameter_size(model_def_path.parent)
    x_l, x_u = expand_parameter_bounds(x_l_raw, x_u_raw, n_x=n_x)
    return {
        "type": DATASET_KIND,
...
def _run_multi_seed(case_dir: Path, data_cfg: dict, cfg_dict: dict, proj_cfg: dict, model_def_path: Path) -> int:
    framework = "nlpopt"
    ...
    _write_local_run_configs(multi_dir, _normalize_local_data_cfg(data_cfg, model_def_path), cfg_dict, proj_cfg, model_def_path)
    ...
    for seed in seeds:
        ...
        artifacts = _run_single_case(case_dir, data_cfg, run_cfg, proj_cfg, model_def_path, output_dir_override=seed_dir)
        manifest = {
            "seed": int(seed),
            "framework": framework,
...
def run_case(_case_dir: Path | None = None, _path_arg: str | None = None) -> int:
    return main(case_dir=_case_dir, path_arg=_path_arg)


def main(*, case_dir: Path | None = None, path_arg: str | None = None) -> int:
    ...
    if unified._str_to_bool(cfg_dict.get("run_multiple_seed", False)):
        return _run_multi_seed(workspace, data_cfg, cfg_dict, proj_cfg, model_def_path)

    artifacts = _run_single_case(workspace, data_cfg, cfg_dict, proj_cfg, model_def_path)
    metadata_path = unified._append_family_metadata(
