...
from scripts.factory.nlp_factory import (  # noqa: E402
...
def _model_dims(model_def, *, n_x: int, param_name: str = "x"):
    y0 = jnp.zeros((model_def.var_spec.total_size,), dtype=model_def.dtype)
    x0 = jnp.zeros((n_x,), dtype=model_def.dtype)
    n_eq = int(model_def.eq_residual({param_name: x0}, y0).shape[0])
    n_ineq = int(model_def.ineq_residual({param_name: x0}, y0).shape[0])
    n_y = int(model_def.var_spec.total_size)
    return n_y, n_eq, n_ineq


def _validate_model_matches_cfg(model_def, data_cfg: dict, *, n_x: int, param_name: str = "x") -> None:
    n_y, n_eq, n_ineq = _model_dims(model_def, n_x=n_x, param_name=param_name)
    if n_y != int(data_cfg["n_y"]) or n_eq != int(data_cfg["n_eq"]) or n_ineq != int(data_cfg["n_ineq"]):
        raise ValueError(
            "Built model does not match data.json: "
            f"expected (n_y={data_cfg['n_y']}, n_eq={data_cfg['n_eq']}, n_ineq={data_cfg['n_ineq']}) "
            f"but got (n_y={n_y}, n_eq={n_eq}, n_ineq={n_ineq})."
        )
...
def _generate_dataset(generator, data_cfg: dict):
    ...
    if str(getattr(generator, "requested_solver", "cvxpy")).lower() == "cvxpy":
        if generator.problem is None:
            generator.build_cvxpy_problem(solver=solver_name)
    ...
    for x in xs:
        ...
        if status in ("optimal", "optimal_inaccurate") and result["y"] is not None:
            ...
            kept_mu.append(coerce_ineq_multipliers(result.get("mu"), n_ineq))
    ...
    Mu = np.stack(kept_mu, axis=0) if kept_mu else np.zeros((X.shape[0], n_ineq), dtype=np.float64)
...
def run_case(
    ...
    *,
    ...
    merged_cfg = {key: value for key, value in cfg_dict.items() if key != "print_every"}
    ...
    _validate_model_matches_cfg(label_model_def, data_cfg, n_x=n_x)
    ...
    _validate_model_matches_cfg(train_model_def, data_cfg, n_x=n_x)
    ...
    Y_train_batches = Y_train.reshape((n_train2 // bs, bs, n_y))
    Y_val_batches = Y_val.reshape((n_val2 // bs, bs, n_y))
    ...
    print(f"JAXMODEL NLP  n_x={n_x} n_y={n_y} n_eq={n_eq} n_ineq={n_ineq}")
    ...
    print("\n=== Supervised evaluation (against label solver) ===")
    ...
    print(f"Relative objective gap: {rel_gap:.6e}\n")
    ...
    if output_dir_override is not None:
        ...
        plot_path = dataset.dataset_dir / "compare_metrics.png"
